...
export const CreatePR = () => {
  ...
  const closeModal = () => {
    const isParentDetailsEmpty = !formState.contact && !formState.ownerFirstName && !formState.ownerLastName && !formState.address && !formState.email;
    const arePetsEmpty = pets.length === 0 || pets.every(pet => !pet.name && !pet.breed && !pet.type && !pet.sex && !pet.dateOfBirth && !pet.uniqueIdentifications && !pet.id);
    
    const isFormEmpty = isParentDetailsEmpty && arePetsEmpty;
    ...
    if (isFormEmpty) {
      ...
      const userConfirmed = window.confirm("If you close the modal, the data will be erased. Do you want to close it anyway?");
      if (userConfirmed) {
        setFormState({
          contact: '',
          ownerFirstName: '',
  ...
  return (
    <>
      {isModalOpen && (
        <div className={isSidebarOpen ? styles.containerOpen : styles.containerClose}>
          <div className={isSidebarOpen ? styles.modalOpen : styles.modalClose}>
            <div className={styles.header}>
              <div className={styles.heading}>Create Pet Record</div>
            ...
            <form className={styles.form}>
              {currentPage === 1 && (
                <div className={styles.fields}>
                  ...
                  <div>
                    <h2 className={styles.formName} style={{ marginTop: '20px' }}>
                      Parent Details
                    </h2>
                  </div>
